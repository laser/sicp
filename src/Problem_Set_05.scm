...
(define beginnings
  '((you say)
    ...
    (i cant help but smile when you say)
    (what makes you say)
    ()))
...
(define general-advice
  '((make sure to take some humanities)
    ...
    (have you thought of something other than school)
    (have you thought of pledging)
    (remember you will never get this tuition money back)
...
;;; They differ in that union will cons an element on the result list
;;; if its found in l2, attempting to produce unique values
;;; ONLY. intersection will only cons an element onto the result list
;;; IF its found in l2.

(list-intersection '(1 2 3) '(1 2 3))
...
;;; Filters out an duplicate symbols. I call it 'unique'
...
;; (I want to take eecs101)
;; (I am scared of phys101)
;; (I was thinking about a seminar)
;; (I would like to take marriage next decade)
;; (I have to double major in ROBOTS and JESUS)
;; (Philip says I should double major as soon as possible)

;; (what is bio101  about)
;; (what are math102 and phys102 about)
;; (how many units is math319)
;; (how many units are bio101 and math102)
;; (what are the prerequisites for eecs202)
;; (can I take eecs202)

;;; Exercise 8

(define (all-prerequisites s dict)
  (define (prereqs? entry)
    (null? (entry-prerequisites entry)))
  (define (loop s d)
              (let ((e (value s d)))
                (if (not (prereqs? e))
                    s
                    (append s
                            (for-each (loop s d) (entry-prerequisites e))))))
  (lambda (dict)
    (reduce list-union
            '()
            (loop 's dict))))
